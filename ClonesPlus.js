...
(function (Scratch) {
    ...
    class ClonesPlus {
      ...
      getVariableOfClone(args, util) {
        let result = [];
        const expectedVarValue = args.INPUTC;
        const main = util.target.sprite.clones[0];
        const clones = main.sprite.clones;
        for (let index = 1; index < clones.length; index++) {
            const checkVar = clones[index].lookupVariableById(args.INPUTB);
            if (checkVar && Scratch.Cast.compare(checkVar.value, expectedVarValue) === 0) {
                const getVar = clones[index].lookupVariableById(args.INPUTA);
                result.push(getVar.value);
            }
        }
        ...
        //   return "";
        // }
        // // guaranteed to exist by getCloneFromVariabsle
      ...
      getThingOfClone(args, util) {
        ...
        for (let index = 1; index < clones.length; index++) {
            ...
            const checkVar = clone.lookupVariableById(args.INPUTB);
            if (checkVar && Scratch.Cast.compare(checkVar.value, expectedVarValue) === 0) {
                result.push(getThingOfTarget(clone, args.INPUTA));
            }
        }
        return JSON.stringify(result);
